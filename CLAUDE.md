...
> **Обновление LU-42 (2026-06-01, commit 8929ac9):** Callback — ОСНОВНОЙ real-time
> канал (`message_new`/`message_reply`), poll — лёгкий backstop, НЕ импортёр истории.
> `poll_branch_messages` теперь `getConversations(filter='unanswered')` +
> гейт `POLL_RECENT_WINDOW_SEC = 30*86400` (не поднимает древний неотвеченный
> бэклог >30д). Это убрало SoftTimeLimit (poll 240с-таймаут → ~12с). Ответы
> менеджера ловит Callback `message_reply` (раньше ради этого держали `filter='all'`,
> но callback-серверы у части тенантов были сломаны — починены, см. ниже).
> Конфиг callback-серверов VK (наш vs Senler, события/секрет/дубли) — память `vk-callback-config`.

- ~~`filter='all', count=50`~~ → `filter='unanswered', count=200` с пагинацией (см. блок выше, LU-42).
...
- **Django 6.0 + `format_html`**: вызов без аргументов-плейсхолдеров (`format_html('<span>...</span>')`) кидает `TypeError: args or kwargs must be provided` — это уже **жёсткая ошибка** (не deprecation). Любое такое поле в `list_display`/readonly → **500** на странице. Баг рецидивирует при добавлении нового кода: RBAC-фича (`a80d0ba`) занесла его в `users/admin.py` (`branch_access_link`/`branch_access_summary`) → 500 на `/admin/users/user/`, починено в `ca84a6a` (2026-06-10). Правило: при добавлении новых badge/display-методов в `@admin.display` — либо ставь `{}` и передавай значения через `format_html`, либо используй `mark_safe` для статической разметки. Прод сейчас на **Django 6.0.3**.
...
- **DRF по умолчанию = AllowAny**: в `REST_FRAMEWORK` нет `DEFAULT_PERMISSION_CLASSES`, поэтому любой новый `APIView` без явного `permission_classes` открыт всему интернету. 09.09.2026 так висели 14 ручек `/api/v1/analytics/*` (включая `rf/send-broadcast` — массовая VK-рассылка), закрыто в `5e18a93`. Правило: каждая сотрудническая ручка получает `permission_classes = [IsAuthenticated]` (JWT мобилки / session веба); гостевые (`_GUEST_PREFIXES`) защищает `VKLaunchParamsMiddleware`, вебхуки — секрет в заголовке. Тест `AnalyticsApiRequiresAuthTest` проверяет все `APIView` модуля analytics — при добавлении новых API-модулей для сотрудников заводить такой же.
...
**Актуальность черновика (09.09.2026, `8b1db12`).** Черновик «свежий», пока
последнее сообщение гостя == `ai_draft_message_id` (старые черновики без
маркера — по `updated_at`). Гость дописал → `_enqueue_ai_draft` снимает
запланированный автоответ (`guest_wrote_again`) и ставит перегенерацию с
дебаунсом `DRAFT_REGEN_DEBOUNCE_SEC=180` (одна задача на тред через
`cache.add`; сама задача идемпотентна — лишний запуск видит свежий маркер и
выходит без ИИ). Лимит `DRAFT_AUTO_GEN_CAP=8` автогенераций на тред
(`ai_draft_auto_generations`), дальше только кнопка «Перегенерировать».
Задача автоотправки несёт `expected_hash`: после перепланирования старая
уходит как `superseded`, шлёт только последняя. Перегенерация НЕ запускается

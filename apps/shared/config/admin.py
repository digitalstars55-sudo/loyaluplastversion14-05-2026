...
class ClientConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Телефон гостя с согласия через ВК (№78)', {
            'fields': ('guest_phone_enabled',),
            'description': (
                'Кнопка «Поделиться номером» в профиле мини-приложения. Гость даёт '
                'телефон через окно согласия ВКонтакте; номер виден в карточке гостя '
                'и уходит в CheckUp с жалобой. Нужен ещё общий выключатель платформы '
                'GUEST_PHONE_ENABLED. Включать по сетям, начиная с пилота.'
            ),
        }),

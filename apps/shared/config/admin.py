    ...
    fieldsets = (
        ...
        ('Авторассылки — предохранитель', {
            ...
            'description': (
                ...
                '0 — без ограничения (по умолчанию, поведение не меняется). '
                'RF-оркестратор — глобальный антиспам v1.1 для RF-правил '
                '(72 часа / 2 за 14 дней / 3 за 30 дней / пауза после визита / '
                'заморозка вокруг ДР).'
            ),
        }),
        ('Телефон гостя с согласия через ВК (№78)', {
            'fields': ('guest_phone_enabled', 'guest_phone_reward_coins'),
            'description': (
                'Кнопка «Поделиться номером» в профиле, форме отзыва и при заборе '
                'суперприза. Гость даёт телефон через окно согласия ВКонтакте; номер '
                'виден в карточке гостя и уходит в CheckUp с жалобой. Нужен ещё общий '
                'выключатель платформы GUEST_PHONE_ENABLED. Баллы за номер начисляются '
                'один раз на гостя; 0 — без награды. Авторассылка «Просьба поделиться '
                'номером» настраивается в конструкторе авторассылок.'
            ),
        }),
        ('Отзывы из ВК: точка по последнему скану', {
            'fields': (
                'vk_review_branch_inference',
                'vk_review_branch_inference_hours',
            ),
            'description': (
                'ВК-группа одна на всю сеть, поэтому у сообщений гостя в группу точки '
                'нет вообще. С флагом мы подставляем точку (и стол) последнего скана QR '
                'в заданном окне — это подсказка, а не выбор гостя: поле «Торговая точка» '
                'у отзыва остаётся пустым, подсказка видна отдельным бейджем «📍 … по скану». '
                'Негатив с такой точкой уходит в жалобы CheckUp с пометкой «по скану». '
                'Окно — насколько старый скан ещё считаем относящимся к сообщению '
                '(по умолчанию 24 часа). По умолчанию выключено — поведение прежнее.'
            ),
        }),
        ('Сетевой вход из каталога VK (новичок без QR)', {
            ...
            'description': (
                'Гость заходит в мини-приложение из каталога VK (без QR) → крутит '
                'колесо → выигрывает приветственный подарок → выбирает свой город → '
                'забирает подарок в любой точке города по коду дня на кассе. '
                'Включите участие, задайте город и отметьте один подарок в каталоге '
                'флагом «Приветственный подарок новичка (VK-каталог)».'
            ),
            'classes': ('collapse',),
        }),
        ('Кассовая система', {

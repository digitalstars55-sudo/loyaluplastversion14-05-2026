...
class ClientConfig(models.Model):
    ...
    # Срок, за который гость обязан ЗАБРАТЬ подарок в кафе (не путать с
    # story_activation_minutes — то окно уже ПОСЛЕ активации).
    story_gift_lifetime_days = models.PositiveSmallIntegerField(
        default=14,
        verbose_name='Срок жизни подарка из сториз/сайта, дней',
        help_text=(
            'Сколько дней у гостя есть, чтобы активировать подарок в кафе. '
            'Отсчёт — от момента сохранения подарка в «Мои подарки». '
            'По истечении подарок сгорает. 0 — бессрочно.'
        ),
    )
    story_gift_reminder_days = models.PositiveSmallIntegerField(
        default=10,
        verbose_name='Напомнить о неактивированном подарке через, дней',
        help_text=(
            'Через сколько дней после получения отправить VK-напоминание, если подарок '
            'так и не активирован. Текст — в «Шаблоны авторассылок» → «Подарок не забран». '
    ...
    # RF-оркестратор (ТЗ авторассылок v1.1 §5): глобальный антиспам для
    # RF-событий (реактивация/welcome/догоняющее). ДР и сервисные сообщения
    # живут своими контурами и им не мешают.
    rf_orchestrator_enabled = models.BooleanField(
        default=False,
        verbose_name='RF-оркестратор (антиспам v1.1)',
        help_text=(
            'Глобальные правила частоты RF-сообщений: минимум 72 часа между ними, '
            'не более 2 за 14 дней и 3 за 30 дней, пауза 7 дней после визита, '
            'заморозка ±7 дней вокруг дня рождения. Выключено — поведение прежнее. '
    ...
    # --- Антихрупкость входа, этап 1 (пер-тенантные флаги; выключено = как раньше) ---
    web_entry_enabled = models.BooleanField(
        default=False,
        verbose_name='Веб-вход вне ВК (через VK ID)',
        help_text=(
            'Мини-приложение работает и вне ВК (браузер, Телеграм) — гость входит '
            'через VK ID и получает тот же профиль и баланс. Выключено — вне ВК '
            'показывается заглушка «Откройте во ВКонтакте», как раньше.'
        ),
    )
    degrade_enabled = models.BooleanField(
        default=False,
        verbose_name='Умная деградация при сбое ВК',
        help_text=(
            'Если внутри ВК механизмы мини-приложения не отвечают — предложить гостю '
            '«Продолжить в браузере» вместо тупиковой ошибки. Требует включённого '
            'веб-входа. Выключено — прежнее поведение.'
        ),
    )

    # --- №78 карты переезда: телефон гостя с согласия через ВК (17.09.2026) ---
    # Пер-тенантный флаг + общий выключатель платформы GUEST_PHONE_ENABLED:
    # кнопка «Поделиться номером» есть только когда включены оба.
    guest_phone_enabled = models.BooleanField(
        default=False,
    ...
    guest_phone_reward_coins = models.PositiveSmallIntegerField(
        default=0,
    ...
    # --- Отзывы из ВК: точка по последнему скану (09.09.2026) ---
    # ВК-группа одна на всю сеть → у сообщений гостя в группу точки нет вообще.
    # Флаг разрешает подставлять точку последнего скана QR как ПОДСКАЗКУ.
    vk_review_branch_inference = models.BooleanField(
        default=False,
        verbose_name='Точка ВК-отзыва по последнему скану',
        help_text=(
            'Для сообщений гостя в ВК-группу подставлять точку (и стол) последнего '
            'скана QR в окне ниже. Негатив с такой точкой уходит в жалобы CheckUp '
            'с пометкой «по скану». По умолчанию выключено.'
        ),
    )
    vk_review_branch_inference_hours = models.PositiveSmallIntegerField(
        default=24,
        verbose_name='Окно поиска скана, часов',
        help_text=(
            'Насколько старый скан ещё считаем относящимся к сообщению. По умолчанию 24.'
        ),
    )

...
class GenerateBroadcastTextAPIView(APIView):
    ...
    def post(self, request):
        ...
        system_prompt = (
            ...
            '- Опирайся на базу знаний заведения ниже: бери из неё акции, '
            'факты и тон общения.\n'
        ...
        # Черновик/пожелания пользователя из поля рассылки — главный приоритет.
        draft = (request.data.get('draft') or '').strip()
        if draft:
            user_message += (
                '\n\n--- Черновик / пожелания пользователя ---\n'
                'Это указание от пользователя — обязательно следуй ему (тема, '
                'акцент, длина, факты). Если это готовый черновик — улучши его, '
                f'сохранив смысл:\n{draft}'
            )

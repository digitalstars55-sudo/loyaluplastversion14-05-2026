...
class BranchConfigInline(admin.StackedInline):
    ...
    fieldsets = (
        ...
        ('Подарок ко дню рождения', {
            ...
            'description': (
                'Сколько дней до и после ДР гость может забрать подарок. '
...
class TestimonialConversationAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        # exclude: треды без сообщений (last_message_at=NULL — мусор старых
        # VK-чисток) прячем — Postgres при DESC-сортировке ставит NULL первыми,
        # и пустые карточки без текста забивали верх списка поверх настоящих.
        return (
...
class ReviewAutoReplyConfigAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Автоподтверждение на негатив (ИИ пишет «спасибо, разберёмся»)', {
            'fields': (
                'auto_ack_enabled',
                'auto_ack_delay_minutes',
                'auto_ack_text',
            ),
            'description': (
                'Полный ответ на негатив по-прежнему пишет человек. Но если за «окно без ответа» '
                'никто не ответил, ИИ отправит гостю короткое подтверждение из поля «Текст подтверждения» — '
                'без обещаний и компенсаций. Тред остаётся неотвеченным, черновик и напоминания работают. '
                'Сотруднику приходит пуш «ИИ напишет в HH:MM» — отменить можно из карточки отзыва. '
                'Точки и лимит в сутки — общие с автоотправкой позитивных. По умолчанию ВЫКЛЮЧЕНО.'
            ),
        }),

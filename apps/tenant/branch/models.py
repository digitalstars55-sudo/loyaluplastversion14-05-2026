...
class TestimonialMessage(models.Model):
    ...
    # Текст сообщения, шедшего ПЕРЕД этим (обычно авто-опрос «Понравилось?» или
    # промо/вопрос менеджера). Заполняется для GUEST-сообщений из VK, когда
    # предыдущее в треде — исходящее (рассылки мы НЕ сохраняем как сообщения,
    # но их текст показываем как цитату-контекст, иначе ответ гостя висит без
    # понимания на что он). reply_to_date — время того сообщения (для UI).
    reply_to_text = models.TextField('Контекст (на что ответ)', blank=True, default='')
    reply_to_date = models.DateTimeField('Время контекста', null=True, blank=True)

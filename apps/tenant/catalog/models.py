...
class Product(TimeStampedModel):
    ...
    is_vk_catalog_welcome = models.BooleanField(
        default=False,
        verbose_name='Приветственный подарок новичка (VK-каталог)',
        help_text=(
            'Подарок, который получает новичок, зашедший в мини-приложение из '
            'каталога VK (без QR) и выбравший этот город. Сетевой: забирается в '
            'любой точке сети по её коду дня. Используется ПЕРВЫЙ активный '
            'подарок с этим флагом. Не смешивается с супер-призами и сториз.'
        ),
    )

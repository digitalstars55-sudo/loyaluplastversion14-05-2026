...
class AutoBroadcastType(models.TextChoices):
    ...
    # сгорания (claim_expires_at). По ТЗ v1.1 §4 — «второй шаг цепочки»:
    # проходит общий RF-антиспам (оркестратор), одно напоминание на подарок.
    RF_GIFT_EXPIRING = 'rf_gift_expiring', 'RF-подарок скоро сгорит (напоминание)'
    # ── №78 (17.09.2026): телефон гостя ──────────────────────────────────────
    # «Через N дней после визита» гостю без номера: сообщение с кнопкой, которая
    # открывает мини-апп на панели согласия; за номер — баллы (ClientConfig).
    PHONE_REQUEST    = 'phone_request',   'Просьба поделиться номером (через N дней после визита)'
...
class AutoBroadcastRule(TimeStampedModel):
    ...
    # ── Фаза 5: подарочный шаг (ТЗ RF-авторассылок §8, §15.4) ─────────────────
    # Пусто — обычное правило без подарка. Задан тир — вместе с отправкой
    # сообщения гостю кладётся подарок из «Каталога наград» (InventoryItem,
    # source=rf_auto) со сроком сгорания; в тексте работают {подарок} и
    # {дней_осталось}. Если подарок выдать нельзя (гейты §8: активный подарок,
    # баланс 3000+, 2 несгоревших RF-подарка за 90 дней, близкий ДР, пустой
    # каталог) — уходит запасной текст без подарка (M0), а при пустом запасном
    # тексте гость просто пропускается до следующего тика.
    gift_tier = models.CharField(
        max_length=16,
        blank=True,
        default='',
        verbose_name='Категория подарка (тир)',
        help_text='Пусто — правило без подарка. Иначе G1, G2, G3 или список '
                  'через запятую (G1,G2) — система сама выберет позицию каталога.',
    )
    gift_lifetime_days = models.PositiveSmallIntegerField(
        default=0,
        verbose_name='Срок сгорания подарка, дней',
        help_text='0 — срок позиции каталога («Срок действия по умолчанию»).',
    )
    gift_fallback_text = models.TextField(
        blank=True,
        default='',
        verbose_name='Запасной текст без подарка (M0)',
        help_text='Уходит, если подарок выдать нельзя. Переменные те же, кроме '
                  '{подарок}/{дней_осталось}. Пусто — гость пропускается.',
    )

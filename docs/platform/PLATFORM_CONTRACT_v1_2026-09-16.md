...
**Что не закрыто и почему:** (1) «сотрудник одной точки не видит другую» через кабинет CheckUp — в CheckUp нет тестовых учёток и роли `client` для `dev`; засчитано по прямому обмену (§7 CheckUp и фикстуры `reviews_list_client_scoped`, `broadcasts_*_client_scoped`, `…_404_foreign`); учётку с правом `loyalty.view` без `loyalty.network` и доступом к точке «Бухгалтерия» (branch 32 ↔ `990002`) заводит владелец. (2) Пишущие проверки на LevOne (ответ гостю в ВК, баллы, рассылка, кампания, правка точки) — по слову владельца на каждую; на `dev` формы ответов записаны в фикстурах. (3) «Токен не виден в браузере» — по устройству BFF CheckUp (обмен и кэш в Redis на сервере); проверку DevTools делает владелец при следующем входе. (4) Карточка отзыва `/loyalty/reviews/{id}` на LevOne — CheckUp снимет отдельно после подключения `GET /mobile/reviews/{id}/`.

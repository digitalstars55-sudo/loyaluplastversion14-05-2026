...
3. **Срезы схемы 18.09** (сгенерированы из кода 2abbc38+): `openapi_w1_2026-09-18.json` + `loyalup_w1_2026-09-18.d.ts` (47 путей волны 1: + `broadcasts/*`, карточка отзыва; лента и сообщения описаны объектами, query-параметры на месте) и `openapi_w2_2026-09-18.json` + `loyalup_w2_2026-09-18.d.ts` (52 пути волны 2: готовые ручки мобилки + все новые модули с `@extend_schema`; старые пути `{rule_id}` мобилки исключены). Причины дефектов 16.09 — декоратор схемы висел на `list`, а не на HTTP-методе, и ListAPIView без `many=False` описывался массивом; исправлено в коде. У готовых ручек мобилки (каталог, квесты, акции, коды дня, ассистент) схемы ответов неполные — форма по `fixtures/w2/`.
...
| 16 | Сотрудники и права | `GET /api/v1/staff/` · `PATCH/DELETE /api/v1/staff/{id}/` · `POST /api/v1/staff/invite/` · `POST /api/v1/staff/link-existing/` | готово | в волне 1 **не переносим**: пользователей, созданных через обмен токена, ведёт CheckUp; ручки нужны только для «родных» сотрудников LoyalUP в переходный период; **с 19.09 запрет серверный**: `PATCH/DELETE /staff/{id}/` и `link-existing/` по личности из обмена (`checkup-*`) → `409 {code: managed_by_checkup}` |
...
| 25 | Дни рождения гостей | `GET /api/v1/guests/birthdays/?days_ahead=&include_past=` | готово | эталоны `guests_birthdays*` (гости обезличены). ⚠️ `include_past` **по умолчанию включён**: параметр не передан или пуст = `1`; скрыть прошедшие — только ровно `include_past=0`. `vk_id` в строке — **строкой** (в точках контакта — числом). `is_loyal` всегда `false` (поле-заготовка). С 19.09 `phone`, `phone_source` (`vk`\|`vk_unverified`\|`review`\|`""`), `segment_emoji`, `segment_name` заполняются как в карточке гостя (пачкой, два запроса на выборку) — до этого приходили пустыми |
...
## 3в. Волна 3: платформенный доступ и сводная по всем клиентам (v1.6, формы на ревью CheckUp)

По карте — №35 «кросс-тенантная сводная», №36 «отзывы всех клиентов», №34 «экономика клиента» (цифры внутри сводной); остальные строки волны 3 (№33 журнал действий, №37 пороги RF, №39 оплата, №38 помощь) — отдельными правками v1.6, когда CheckUp дойдёт до экранов.

### 3в.1. Платформенный доступ через обмен токена
...
- **v1.7:** признак приходит и в теле обмена — `platform: true` (строго булево `true`; CheckUp ставит его по праву `loyalty.platform`, которое владелец выдаёт сам в редакторе ролей). Итог: `platform = body.platform is True ИЛИ id ∈ CHECKUP_PLATFORM_ADMINS`; env остаётся аварийным списком. Полю можно верить: запрос подписан секретом обмена и принимается только с внутреннего адреса.
...
Обе ручки **уже есть** на публичной схеме (мобильное приложение суперадмина); в волне 3 меняется только гейт: `is_superuser` **или** `platform: true` в JWT. Ходить с `Host: levelupapp.ru` и платформенным JWT любой сети (сеть в токене не важна).
...
### 3в.4. №31 — «предпросмотр аудитории словами» (для маркетолога)

`GET /api/v1/auto-broadcasts/{id}/preview/` дополняется полем `explanation` (текст, 2–4 предложения): кто получит и почему — событие с задержкой словами («гости, чей последний визит был ровно 30 дней назад»), точки/пол/сегменты, что исключает общий дедуп (в этом году / сегодня / по объекту), недельный кэп сети и RF-оркестратор, окно отправки по МСК и почему правило сейчас не сработает (`reason`). Плюс `events[].description` (уже есть) и `rule.audience_summary`/`reward_summary` (уже есть). Цифры отсева не даём (движок их не выдаёт). Подписи условий на стороне LoyalUP — человеческие, CheckUp показывает как есть.

### 3в.5. Оценка, «никогда», приёмка

| Работа | Оценка |
|---|---|
| Белый список, любая живая сеть для него, `platform: true` в JWT и ответе обмена, тесты | 1 день |
...
Приёмка: обмен под `checkup_user_id=1` на `levone` и на не переехавшую сеть → `200` с `platform: true`; под обычным id на не переехавшую → `403 tenant_not_allowed`; `internal/tenants/` → живые сети с `exchange_open`; `overview/stats/` и `overview/reviews/` под платформенным JWT → `200`, под обычным → `403`; ответ на отзыв другой сети через обмен на неё — `201`.

**Приёмка пройдена 19.09** (CheckUp через BFF, по слову владельца): волна 3 на проде с 4d507f0 (~03:00 MSK) — все шесть пунктов; v1.7 (`platform` из тела) на проде с cd3223d (21:07 MSK): user без права и вне env → `platform: false`; владелец с правом → `true`; под токеном без права сводная → `403`. Белый список env: `1,69` (владелец, Антон); право `loyalty.platform` Антону назначает владелец в CheckUp.

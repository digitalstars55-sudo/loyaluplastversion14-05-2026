# Контракт платформы CheckUp × LoyalUP — v1.1

**Дата:** 16 сентября 2026 (v1), правки v1.1 — тот же день вечером по ревью агента CheckUp · **Статус:** согласован обеими сторонами с правками; подпись владельца · **Для кого:** команда (агент) CheckUp, строящая модуль «Гости → LoyalUP»; владелец платформы
...
**Контрактные тесты вместо ревью кода.** Односторонний ревью агентами не масштабируется, тесты — да. LoyalUP публикует OpenAPI-срез волны 1 и пакет записанных ответов песочницы (JSON по 12 роутам приёмки, включая ошибки 401/403/409/422); CheckUp гоняет свой BFF против этих записей в CI. LoyalUP держит у себя тесты формы ответов тех же ручек — изменение формы ломает тест раньше, чем сломает CheckUp. Правило: форма ответа меняется только аддитивно, удаление поля — новая версия контракта.
...
*Документ живёт в `backend/docs/platform/PLATFORM_CONTRACT_v1_2026-09-16.md`; артефакт — копия. Изменения — только через сторону LoyalUP, с пометкой версии. v1 — 16.09.2026; v1.1 — 16.09.2026 вечер, по ревью агента CheckUp.*
